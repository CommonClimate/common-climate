function varargout = wt_rect(d,varargin)
%% Continous Wavelet Transform, rectified coefficients
% Creates a figure of wavelet power in units of
% normalized variance.
%
% USAGE: [wave,period,scale,coi,sig95]= wt_rect (d[,settings])
% 
% d: a time series
...
%
%  Rectified version using Liu, Yonggang, X. San Liang, Robert H. Weisberg, 2007: 
%  Rectification of the bias in the wavelet power spectrum. J. Atmos. Oceanic Technol., 24, 2093?2102.
%  doi: http://dx.doi.org/10.1175/2007JTECHO511.1
%  
%  Written Jan 17 2013 based on wt.m by Aslak Grinstead (see URL above) 
...
scale2 = repmat(scale,[n 1]);
t=d(:,1);
power = ((abs(wave)).^2)./scale2' ;        % compute wavelet power spectrum
...
if Args.MakeFigure
    ...
    if Args.BlackandWhite
        ...
        %H=contourf(t,log2(period),log2(abs(power/sigma2)));
